...
#https://github.com/flask-extensions/Flask-SimpleLogin documentação do Flask-SimpleLogin
import os

app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///padaria.db"
app.config['SECRET_KEY'] = 'MinhaChaveSecreta'
app.config['SIMPLELOGIN_USERNAME'] = 'gustavo'
app.config['SIMPLELOGIN_PASSWORD'] = '12345678'
db = SQLAlchemy()
db.init_app(app)
SimpleLogin(app)

def validar_imagem(nome_imagem: str) -> bool:
    ...
...
def listar_produtos():
    if request.method == 'POST':
        ...
        resultado = db.session.execute(db.select(Product).filter(Product.nome.like(f'%{termo}%'))).scalars().all()
        return render_template('produtos.html', produtos=resultado)

    else:
        produtos = db.session.execute(db.select(Product)).scalars().all()
...
def cadastrar_produto():
    if request.method == 'POST':
        ...
        dados = request.form
        try:
            produto = Product(nome=dados['nome'], descricao=dados['descricao'], ingredientes=dados['ingredientes'], origem=dados['origem'], imagem=imagem.filename)
            imagem.save(os.path.join('static/imagens', imagem.filename))
            db.session.add(produto)
            db.session.commit()
        except Exception as e:
            status = {"type" : "erro", "mensagem" : f"Erro ao cadastrar produto: {str(e)}"}

        
        return render_template('cadastrar.html', status = status)
    
    else:
        return render_template('cadastrar.html', status = None)
...
def editar_produtos(id):
    if request.method == 'POST':
        ...
        imagem = request.files['imagem']
        produto = db.session.execute(db.select(Product).filter_by(id=id)).scalar()
        ...
        produto_editado = db.session.execute(db.select(Product).filter_by(id=id)).scalar()
        return render_template('editar.html', produto=produto_editado)
    
@app.route('/deletar_produto/<int:id>', methods=['GET'])
@login_required
def deletar_produto(id):
    produto_deletado = db.session.execute(db.select(Product).filter_by(id=id)).scalar()
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
        # app.run(debug=True) remover para deploy